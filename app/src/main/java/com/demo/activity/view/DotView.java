...
public class DotView extends View {
    ...
    private void init() {
        ...
        mDotPaint.setTextSize(ViewUtil.sp2px(mContext, 8));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        int width;
        if (isShowJustDot()) {
            width = MeasureSpec.makeMeasureSpec(2 * ViewUtil.dip2px(mContext, 3), MeasureSpec.EXACTLY);
        } else {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //画点
        if (isShowJustDot) {
            canvas.drawCircle(circleCenter.x + ViewUtil.dip2px(mContext, 3), circleCenter.y + ViewUtil.dip2px(mContext, 3), ViewUtil.dip2px(mContext, 3), mDotPaint);
            return;
        }
        updataPointF();
    ...
    private void drawNumText(Canvas canvas) {
        ...
        canvas.drawText(showNum, textPoint.x - (showNum.contains("1") ? ViewUtil.dip2px(mContext, 1) : 0), textPoint.y, mDotPaint);
        mDotPaint.setColor(Color.RED);
        mDotPaint.setStyle(Paint.Style.FILL);
    }
    ...
    private void setPaddingValue(int num) {
        if (num == 1) {
            padding = ViewUtil.dip2px(mContext, 4);
            mDotPaint.setTextSize(ViewUtil.sp2px(mContext, 9));
        } else if (num > 0 && num < 10) padding = ViewUtil.dip2px(mContext, 4);
        else if (num > 99) padding = 2f;
        else padding = ViewUtil.dip2px(mContext, 3);
    }
